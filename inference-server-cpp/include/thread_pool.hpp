...
private:
    ...
    std::atomic<float> latest_val{0.0f}; 
   ...
   void worker_loop(int id) {
        while (true) {
                ...
                if (wait_time > 100) {
                    ...
                    if(requests[i].promise) {
                        try {
                            ...
                            requests[i].promise->set_value({{}, true});
                        } catch (...) {} 
                    }
                } else {
            ...
            try {
                ...
                std::vector<int64_t> batch_type_ids(batch_tokens.size(), 0);
                ...
                Ort::Value type_ids_tensor = Ort::Value::CreateTensor<int64_t>(memory_info, batch_type_ids.data(), batch_type_ids.size(), shape.data(), shape.size());

                const char* input_names[] = {"input_ids", "attention_mask", "token_type_ids"};
                
                const char* output_names[] = {"last_hidden_state"}; 
                Ort::Value input_tensors[] = {std::move(ids_tensor), std::move(mask_tensor), std::move(type_ids_tensor)};
...

                {
                    ...
                    if (latency_samples.size() > 1000) latency_samples.pop_front();
                }
                ...
                for (int b = 0; b < actual_batch_size; ++b) {
                    ...
                    for (int t = 0; t < sequence_length; ++t) {
                        for (int d = 0; d < embedding_dim; ++d) {
                            ...
                            final_embedding[d] += output_data[(b * sequence_length * embedding_dim) + (t * embedding_dim) + d];
                        }
                    }
                    ...
                    if (requests[original_index].promise) {
                        try {
                            requests[original_index].promise->set_value({final_embedding, false});
                        } catch (...) {}
                    }

                    if (b == 0) {
                        std::cout << "[WORKER " << id << "] Embedding Generated | Dim: " << final_embedding.size() 
                                  << " | Latency: " << std::chrono::duration<double, std::milli>(end_infer - start_infer).count() << "ms" << std::endl;
                    }
                }
...
public:
    ...
    float get_latest_val() const { return latest_val.load(); }
